 ...
 *    Local PMC adaptation, 2026-09-14: require optimal status for every LP;
 *    fail on missing objective values; disable shared CPLEX logfile writes.
 *    Mathematical formulation and basis-reuse algorithm are unchanged.
      ...
      for (k = 0; k < nrxn; k++)
      {
        ...
        if (performOptim) {
          ...
          */
          ...
          /* CPXlpopt returning zero does not imply optimality (e.g. time limit).
             Check even when the caller requests only the four small outputs. */
          if (CPXgetstat(env, lp) != CPX_STAT_OPTIMAL) {
              mexPrintf("PMC: reaction %i round %i is not optimal (status %i).\n",
                        j, iRound, CPXgetstat(env, lp));
              return FVA_MODIFIED_FAIL;
          }
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    int             opt_logfile = 0;    /* worker-owned MATLAB logs, no shared file */
